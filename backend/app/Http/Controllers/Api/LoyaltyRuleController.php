...
class LoyaltyRuleController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (in_array($request->rule_type, ['purchase_based', 'birthday_bonus', 'referral_bonus'])) {
            if (!$request->amount_per_point) {
                return response()->json(['error' => 'amount_per_point is required for purchase-based, birthday bonus, or referral bonus rules'], 400);
            }
        } else {
    ...
    public function calculatePointsForPurchase(Request $request)
    {
        ...
        $loyaltyPrograms = LoyaltyProgramRule::where('loyalty_program_id', $request->loyalty_program_id)
    ...
    public function awardPoints(Request $request)
    {
        ...
        if ($calculationData->total_points_earned > 0) {
            DB::transaction(function() use ($user, $calculationData) {
